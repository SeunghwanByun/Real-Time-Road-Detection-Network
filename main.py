...
from CAR import CAR
from utils import acc_check, value_tracker, poly_learning_rate
...
def main():
    ...
    for epoch in range(epochs):
        ...
        for i, data in enumerate(dataset_loader, 0):
            ...
            if i % 20 == 19:
                value_tracker(vis, torch.Tensor([i + epoch * len(dataset_loader)]), torch.Tensor([running_loss / 10]),
                              loss_plt)
                print("[%d, %5d] loss: %.3f lr: %f, time: %.3f" % (
                epoch + 1, i + 1, running_loss / 10, optimizer.param_groups[0]['lr'], time.time() - start))
                start = time.time()
        ...
        if epoch == 0 or epoch % 100 == 99:
            ...
            torch.save(model.state_dict(), "./model/model_epoch_{}.pth".format(epoch + 1))
